...
export function createSwordPhysics(sword: THREE.Group, onStatus: (status: MotionStatus) => void) {
  ...
  collider(.83,.05,.06,0,.22);
  collider(.12,.88,.10,-.96,.12);
  collider(.212,.212,.085,-2.035,.18);
  ...
  const shell=createScabbardGeometry();
  ...
  function position(){return origin.clone().addScaledVector(axis,draw*DRAW_DISTANCE).multiplyScalar(scale)}
  ...
  function restore(){released=false;draw=target;body.setBodyType(RAPIER.RigidBodyType.KinematicPositionBased,true);body.setTranslation(position(),true);body.setRotation(rotation,true);body.setLinvel({x:0,y:0,z:0},true);body.setAngvel({x:0,y:0,z:0},true);body.setNextKinematicTranslation(position());body.setNextKinematicRotation(rotation);accumulator=0;capture();previousPosition.copy(currentPosition);previousRotation.copy(currentRotation);sync();notify(draw>=.999?'drawn':draw<=.001?'sheathed':'drawing')}
  function step(delta:number){
    accumulator+=Math.min(delta,.1);
    while(accumulator>=world.timestep){
      if(!released){draw=THREE.MathUtils.damp(draw,target,5,world.timestep);if(Math.abs(draw-target)<.0005)draw=target;body.setNextKinematicTranslation(position());body.setNextKinematicRotation(rotation);notify(draw>=.999?'drawn':draw<=.001?'sheathed':'drawing')}
