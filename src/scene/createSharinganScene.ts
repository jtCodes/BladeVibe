...
import {SHARINGAN_ENVIRONMENT} from './sceneEnvironmentPresets';
...
export function createSharinganScene(container:HTMLElement,preview=false){
 ...
 const environment=createSceneEnvironment(scene,renderer,SHARINGAN_ENVIRONMENT);
 ...
 reset();
 return {setActive,setSpinning(value:boolean){spinning=value;},reset,dispose(){setActive(false);observer.disconnect();controls.dispose();atmosphere.dispose();environment.dispose();model.dispose();renderer.dispose();renderer.domElement.remove();}};
}
