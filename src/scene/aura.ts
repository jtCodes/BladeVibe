...
export function createBladeAura(sword:THREE.Group, pixelRatio:number) {
 ...
 const electric=createBladeElectric(group,random);
 ...
 const material=new THREE.ShaderMaterial({uniforms,transparent:true,depthWrite:false,blending:THREE.AdditiveBlending,
  ...
  fragmentShader:`uniform float electric;uniform float flame;uniform float ice;uniform float intensity;uniform float time;varying float opacity;varying float tint;
   ...
   void main(){float r=length(gl_PointCoord-.5)*2.;if(r>1.||opacity<.001)discard;
    ...
    vec3 color=mix(vec3(.18,.62,1.),vec3(.70,.90,1.),tint);
    {
     ...
     color=mix(color,vec3(8.,7.4,5.8),electric);
 ...
 function spawn(exposed:number){
  ...
  if(mode==='electric'){velocity.set((random()-.5)*1.7,(random()-.5)*1.2,(random()-.5)*1.7).applyQuaternion(rotation);rawSpeed.lerpVectors(rootSpeed,tipSpeed,y/5);velocity.addScaledVector(rawSpeed,.65);}
  origin.toArray(positions,i*3);velocity.toArray(velocities,i*3);
  if(mode==='ice')iceChips.spawn(i);
  ages[i]=0;lifetimes[i]=mode==='electric'?.12+random()*.25:mode==='flame'||mode==='ice'?1.2+random()*.7:.65+random()*.65;
 }
 return {configure(next:EffectMode,nextSpeed:number,nextIntensity:number){
  ...
  if(uniforms.exposed.value>.23&&uniforms.motion.value&&mode!=='off'){
   emission+=dt*uniforms.intensity.value*(mode==='ice'?iceStress:mode==='flame'?180:mode==='electric'?65:190)*THREE.MathUtils.clamp(draw,0,1);
   if(mode==='ice'){
