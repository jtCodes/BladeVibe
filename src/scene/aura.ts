...
export type EffectMode='off'|'glow'|'flame';
...
export function createBladeAura(sword:THREE.Group, pixelRatio:number) {
 ...
 const uniforms={intensity:{value:1},upLocal:{value:new THREE.Vector3(0,-1,0)},lagRoot:{value:new THREE.Vector3()},lagTip:{value:new THREE.Vector3()},flame:{value:1},time:{value:0},exposed:{value:0},pixelRatio:{value:pixelRatio},floorY:{value:FLOOR_Y},motion:{value:window.matchMedia('(prefers-reduced-motion: reduce)').matches?0:1}};
 ...
 for(const face of [-1,1])for(const halo of [false,true]) {
  ...
  const material=new THREE.ShaderMaterial({uniforms:{...uniforms,strength:{value:halo?.19:1.25}},transparent:true,depthWrite:false,side:THREE.DoubleSide,blending:THREE.AdditiveBlending,
   ...
   fragmentShader:`uniform float time;uniform float exposed;uniform float strength;uniform float motion;uniform float flame;uniform float intensity;varying vec2 vUv;varying float bladeY;
    void main(){float reveal=1.-smoothstep(exposed-.04,exposed,bladeY);
     ...
     gl_FragColor=vec4(mix(vec3(.16,.65,1.15),vec3(5.,1.2,.08),flame)*strength*intensity,edge*ends*reveal*pulse);}`});
 ...
 const material=new THREE.ShaderMaterial({uniforms,transparent:true,depthWrite:false,blending:THREE.AdditiveBlending,
  vertexShader:`attribute float seed;attribute float alpha;uniform float pixelRatio;uniform float flame;uniform float intensity;varying float opacity;varying float tint;
   void main(){vec4 view=modelViewMatrix*vec4(position,1.);
    opacity=alpha;tint=seed;gl_Position=projectionMatrix*view;
    gl_PointSize=clamp(mix(1.8+seed*1.8,4.+seed*3.,flame)*pixelRatio*14./max(1.,-view.z),1.,128.*pixelRatio);
   }`,
  fragmentShader:`uniform float flame;uniform float intensity;uniform float time;varying float opacity;varying float tint;
 ...
 function trackMotion(dt:number){
  ...
  if(jump){rootSpeed.set(0,0,0);tipSpeed.set(0,0,0);motionReady=true;}
  else if(dt>0){
 ...
 function spawn(exposed:number){
  ...
  ages[i]=0;lifetimes[i]=mode==='flame'?1.2+random()*.7:.65+random()*.65;
 }
 return {configure(next:EffectMode,nextSpeed:number,nextIntensity:number){
  if(next!==mode){ages.fill(10);opacities.fill(0);emission=0;geometry.attributes.alpha.needsUpdate=true;}
  uniforms.intensity.value=THREE.MathUtils.clamp(nextIntensity,0,2);
  mode=next;speed=THREE.MathUtils.clamp(nextSpeed,0,3);uniforms.flame.value=mode==='flame'?1:0;
 },update(dt:number,draw:number){
  trackMotion(dt);
  dt*=speed*uniforms.motion.value;particles.visible=mode!=='off'&&uniforms.intensity.value>0;continuousFire.visible=mode==='flame';
