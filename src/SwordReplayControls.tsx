...
export function SwordReplayControls({sword,sceneRef,visible,effect,selected,paused,speed,intensity,onSelect,onReplay,onPause,onSeek,onOriginal,options}:Props){
 ...
 if(sword.model==='zangetsu')return <div className="replay-controls simple-controls">
  <div className="form-selector"><button aria-pressed={effect!=='bankai'} onClick={onOriginal}>Shikai</button><button aria-pressed={effect==='bankai'} onClick={()=>onSelect('bankai')}>Bankai</button></div>{options}
 </div>;
 if(!timed)return <div className="replay-controls simple-controls"><PlaybackActions paused={stopped} onToggle={onPause} playLabel="Play effect" pauseLabel="Pause effect"/>{options}</div>;
 ...
 return <div className="replay-controls">
  ...
  <div className="replay-tools"><PlaybackActions paused={stopped} onToggle={!playing?onReplay:onPause} onReplay={onReplay} playLabel="Play effect" pauseLabel="Pause effect"/>{options}</div>
