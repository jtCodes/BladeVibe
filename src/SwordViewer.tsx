...
export function SwordViewer({ viewState, active=true, sceneRef, effectSeek, effectPaused=false, bankaiPetalMotion='drift', model='longsword', glowStrength=.42, glowSpread=.8, petalGlow=4, upscaling='native', dragTarget='sword', antiAliasing='smooth', showPerformance=false, floorColor, backgroundColor, lighting, rotating, draw, reflections, lightAngle, cameraHeight,showSheath=false,swordRotation=0,effect,effectSpeed,effectIntensity, resetVersion, dropVersion, onStatus }: Props) {
 ...
 const settings=useRef({viewState,effectSeek,effectPaused,bankaiPetalMotion,glowStrength,glowSpread,petalGlow,upscaling,dragTarget,antiAliasing,showPerformance,lighting,rotating,draw,reflections,lightAngle,floorColor,backgroundColor,cameraHeight,showSheath,swordRotation,effect,effectSpeed,effectIntensity});settings.current={viewState,effectSeek,effectPaused,bankaiPetalMotion,glowStrength,glowSpread,petalGlow,upscaling,dragTarget,antiAliasing,showPerformance,lighting,rotating,draw,reflections,lightAngle,floorColor,backgroundColor,cameraHeight,showSheath,swordRotation,effect,effectSpeed,effectIntensity};
 useEffect(()=>{
 ...
 useEffect(()=>{scene.current?.update({viewState,effectSeek,effectPaused,bankaiPetalMotion,glowStrength,glowSpread,petalGlow,upscaling,dragTarget,antiAliasing,showPerformance,lighting,rotating,draw,reflections,lightAngle,floorColor,backgroundColor,cameraHeight,showSheath,swordRotation,effect,effectSpeed,effectIntensity})},[viewState,effectSeek,effectPaused,bankaiPetalMotion,glowStrength,glowSpread,petalGlow,upscaling,dragTarget,antiAliasing,showPerformance,lighting,rotating,draw,reflections,lightAngle,floorColor,backgroundColor,cameraHeight,showSheath,swordRotation,effect,effectSpeed,effectIntensity]);
