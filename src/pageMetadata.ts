...
const home:PageMetadata={path:'/',title:'BladeVibe — Legendary swords in high-quality 3D',description:'Explore legendary swords in high-quality, real-time 3D. Release Senbonzakura Bankai, reveal Tensa Zangetsu, customize the lighting, and share your scene.',index:true};
export const publicPages:PageMetadata[]=[home,...swords.flatMap(sword=>{
 const path=swordUrl(sword);
 const base={path,title:`${sword.name} — Interactive 3D sword | BladeVibe`,description:`Explore ${sword.name} in high-quality, real-time 3D. ${sword.description} Adjust the scene and share your view.`,index:true};
 return [base,...(['shikai','bankai'] as const).filter(form=>swordFormEffect(sword,form)!==undefined).map(form=>({
  path:`${path}/${form}`,index:true,
  title:sword.id==='zangetsu'&&form==='bankai'?'Tensa Zangetsu — Bankai in 3D | BladeVibe':`${sword.name} ${form==='bankai'?'Bankai':'Shikai'} — Interactive 3D | BladeVibe`,
...
}),{path:'/sharingan',title:'Sharingan — Interactive 3D study | BladeVibe',description:'Explore a monumental crimson Sharingan in real-time 3D. Look around, approach the three-tomoe symbol, and pause its rotation.',index:true}];
export function pageMetadata(path:string):PageMetadata{
 ...
 if(sword)return {...publicPages.find(page=>page.path===swordUrl(sword))!,title:`${sword.name} · Editor — BladeVibe`,index:false};
 return {path,title:'Page not found — BladeVibe',description:'This BladeVibe page could not be found. Explore the sword collection.',index:false};
}
export function updatePageMetadata(path:string){
 ...
 const origin=document.head.querySelector<HTMLMetaElement>('meta[name="bladevibe-site-url"]')?.content;
 if(origin){
