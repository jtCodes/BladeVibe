...
export default function App(){
 ...
 const home=path==='/';
 const ocular=path==='/studies/sharingan';
 ...
 useEffect(()=>{document.title=ocular?'Sharingan — BladeX':home?'The collection — BladeX':sword?`${sword.name}${editing?' · Editor':form?` · ${form==='bankai'?'Bankai':'Shikai'}`:''} — BladeX`:'Study not found — BladeX';},[home,sword,path]);
 return <>
  <div className="session-page" hidden={!home} inert={!home}>{(home||visitedHome)&&<Gallery swords={swords} active={home}/>}</div>
  <div className="session-page" hidden={!sword} inert={!sword}>{retained&&<Suspense fallback={<main className="opening-page"><AppLink href="/" className="text-link">The collection</AppLink><p role="status">Opening the study…</p></main>}><Experience key={retained.sword.id} sword={retained.sword} active={!!sword} editing={retained.editing} routeForm={retained.form} shareHash={retained.hash}/></Suspense>}</div>
  <div className="session-page" hidden={!ocular} inert={!ocular}>{(ocular||visitedOcular)&&<Suspense fallback={<p role="status">Opening the study…</p>}><Sharingan active={ocular}/></Suspense>}</div>
  {!home&&!sword&&!ocular&&<main className="missing-sword"><p className="eyebrow">BladeX</p><h1>Study not found.</h1><AppLink href="/" className="text-link">Return to the collection</AppLink></main>}
 </>;
}
