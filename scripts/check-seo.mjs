...
const output=fs.mkdtempSync(path.join(os.tmpdir(),'bladex-seo-'));
try{
 fs.copyFileSync('index.html',path.join(output,'index.html'));
 const generate=env=>execFileSync(process.execPath,['scripts/build-seo.mjs',output],{env:{...process.env,SITE_URL:'https://bladex.example',SOCIAL_IMAGE_URL:'https://bladex.example/social/cover.png',...env},stdio:'pipe'});
 ...
 for(const page of publicPages){
  ...
  assert.ok(html.includes(`href="https://bladex.example${page.path}"`));
  assert.ok(html.includes(`property="og:url" content="https://bladex.example${page.path}"`));
  assert.ok(html.includes('property="og:image"'));
  assert.ok(html.includes('content="index, follow"'));
  assert.ok(sitemap.includes(`<loc>https://bladex.example${page.path}</loc>`));
 }
 ...
 generate({SITE_URL:'',VERCEL_PROJECT_PRODUCTION_URL:'bladex-production.vercel.app',SOCIAL_IMAGE_URL:''});
 assert.match(fs.readFileSync(path.join(output,'index.html'),'utf8'),/https:\/\/bladex-production.vercel.app\//);
 ...
 assert.throws(()=>generate({SITE_URL:'https://bladex.example/not-an-origin'}));
